...
entry fun create_proposal(description: std::string::String, ctx: &mut iota::tx_context::TxContext) {
...
entry fun vote(proposal: &mut Proposal, choice: bool, ctx: &mut iota::tx_context::TxContext) {
    let sender = ctx.sender();
...
entry fun close_proposal(proposal: &mut Proposal, ctx: &iota::tx_context::TxContext) {
    let sender = ctx.sender();
